...
import { collection, getDocs, updateDoc, doc, setDoc, getDoc, addDoc, query, where, limit } from 'firebase/firestore';
...
const VendorVerification = () => {
  ...
  useEffect(() => {
    ...
    const fetchPendingVendors = async () => {
      try {
        const querySnapshot = await getDocs(collection(rentmobileDb, 'Vendorusers'));
        const allVendors = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const pendingVendors = allVendors.filter(vendor => vendor.status?.toLowerCase() === 'pending');
        ...
        <VendorListContainer>
          {filteredVendors.map(vendor => (
            <VendorItem key={vendor.id}>
              <VendorInfo onClick={() => navigate(`/edit-verification/${vendor.id}`)}>
                {vendor.firstName} {vendor.middleName} {vendor.lastName}
              </VendorInfo>
              <div>
                <ActionButton
