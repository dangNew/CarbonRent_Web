...
import { FaBars, FaSearch, FaEye, FaCheckCircle, FaFilter } from 'react-icons/fa';
...
const SettledButton = styled.button`
  background-color: green;
  color: white;
  border: none;
  padding: 6px 14px;
...
const ViolationPending = () => {
  ...
  const handleSearch = (term) => {
    ...
    if (filter) {
      filtered = filtered.filter(violation => violation.warning === filter);
    }

    setFilteredViolations(filtered);
  };

  const handleFilterWarning = (filter) => {
    setFilter(filter);
    let filtered = violations.filter(violation =>
      (violation.warning?.toLowerCase().includes(filter.toLowerCase()) ||
      violation.violationType?.toLowerCase().includes(filter.toLowerCase()) ||
      violation.message?.toLowerCase().includes(filter.toLowerCase()))
    );
  ...
  const handleOpenModal = (violation) => {
    setSelectedImage(violation.image_0);
  ...
  const handleSettleViolation = async () => {
    if (selectedViolation) {
      const docRef = doc(rentmobileDb, 'Market_violations', selectedViolation.id);
      await updateDoc(docRef, {
        status: 'Settled', // Update status to Settled
      });
      console.log('Settling violation:', selectedViolation);
      handleCloseModal();
      fetchViolations(); // Re-fetch the violations to reflect the changes
    }
  };
  ...
  const fetchViolations = async () => {
    try {
      ...
      const q = query(violationCollection, where('vendorId', '==', vendorId), where('status', '==', 'Pending'));
  ...
  return (
    <DashboardContainer>
      ...
      <MainContent isSidebarOpen={isSidebarOpen}>
        ...
        <SummaryContainer>
          <ControlsContainer>
            <p>{filteredViolations.length} Pending Violations</p>
            ...
            <tbody>
              {filteredViolations.map((violation) => (
                <tr key={violation.id}>
                  ...
                  <td>
                    <ImageButton onClick={() => handleOpenModal(violation)}>
                      <FaEye className="icon" /> View Image
                    </ImageButton>
                  </td>
                </tr>
              ))}
        ...
        <Modal open={openModal} onClose={handleCloseModal}>
          <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'background.paper', boxShadow: 24, p: 4 }}>
            ...
            {selectedViolation && selectedViolation.warning === 'Final Offense' && (
              <SettledButton onClick={handleSettleViolation} sx={{ marginTop: 2 }}>
                <FaCheckCircle className="icon" /> Settled
              </SettledButton>
            )}
          </Box>
        </Modal>
...
export default ViolationPending;
