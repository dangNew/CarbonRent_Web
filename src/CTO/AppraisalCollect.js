    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        const appraisalsQuery = query(
          collection(rentmobileDb, 'appraisals'),
        ...
        if (!appraisalsSnapshot.empty) {
          const appraisalsData = appraisalsSnapshot.docs.map(doc => ({
            ...
            date: doc.data().created_date ? new Date(doc.data().created_date.seconds * 1000) : new Date(),
            createdDate: doc.data().created_date ? new Date(doc.data().created_date.seconds * 1000).toLocaleDateString() : 'N/A',
            location: doc.data().appraisal_assign || 'N/A', // Fetch location
          }));

          // Fetch collector details for each appraisal
          const collectorIds = [...new Set(appraisalsData.map(appraisal => appraisal.appraisal))];
          const collectorDetailsMap = {};

          for (const collectorId of collectorIds) {
            const collectorQuery = query(
              collection(rentmobileDb, 'appraisal_user'),
          ...
          const filteredItems = appraisalsData.filter(item => collectorDetailsMap[item.appraisal]?.firstname + ' ' + collectorDetailsMap[item.appraisal]?.lastname === collector);
          setItems(filteredItems);

          // Set collector details
          const collectorDetail = collectorDetailsMap[filteredItems[0]?.appraisal];
          setCollectorDetails({
  ...
  return (
    <div style={{ textAlign: "center", padding: "20px", fontFamily: "Arial, sans-serif" }}>
      ...
      <div ref={pdfRef}>
        ...
        <h2 style={{ margin: "20px 0", fontSize: "18px", fontWeight: "bold" }}>Appraisal Report</h2>

        <div style={{ margin: "20px 0", textAlign: "left", paddingLeft: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ margin: 0, fontWeight: "bold" }}><strong>Collector ID:</strong> {collectorDetails?.id || "N/A"}</p>
            <p style={{ margin: 0, fontWeight: "bold" }}><strong>Name:</strong> {collectorDetails?.name || "Loading..."}</p>
            <p style={{ margin: 0, fontWeight: "bold" }}><strong>Location:</strong> {collectorDetails?.location || "Loading..."}</p>
          </div>
        </div>
          ...
          >
            <thead>
              <tr>
                ...
                <th style={{ border: "1px solid #ddd", padding: "8px", backgroundColor: "#f2f2f2" }}>Item Code</th>
                ...
                <th style={{ border: "1px solid #ddd", padding: "8px", backgroundColor: "#f2f2f2" }}>Total Amount</th>
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={index}>
                  <td style={{ border: "1px solid #ddd", padding: "8px" }}>{item.createdDate}</td>
                  <td style={{ border: "1px solid #ddd", padding: "8px" }}>{item.code}</td>
                  <td style={{ border: "1px solid #ddd", padding: "8px" }}>{item.goods_name}</td>
                  <td style={{ border: "1px solid #ddd", padding: "8px" }}>{item.unit_measure}</td>
