...
const Transactions = () => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const combinedData = paymentsData.map((payment) => ({
          ...payment,
          location: approvedVendorsData[payment.vendorID]?.location || 'Unknown',
          billingCycle:
  ...
  const filterTransactions = (searchValue, dateValue, unitValue, filterValue) => {
    let filtered = recentTransactions.filter(transaction => {
      const formattedTransactionDate = formatDate(transaction.paymentDate);
      return (
        (transaction.name && transaction.name.toLowerCase().includes(searchValue.toLowerCase())) &&
    ...
    if (filterValue === 'Daily') {
      filtered = filtered.filter(transaction => transaction.billingCycle.toLowerCase() === 'daily');
    } else if (filterValue === 'Weekly') {
      filtered = filtered.filter(transaction => transaction.billingCycle.toLowerCase() === 'weekly');
    } else if (filterValue === 'Monthly') {
      filtered = filtered.filter(transaction => transaction.billingCycle.toLowerCase() === 'monthly');
    }
  ...
  const generatePDF = async () => {
    ...
    const groupedTotalAmounts = Object.entries(groupedTransactions).map(([date, transactions]) => {
      const total = transactions.reduce((sum, transaction) => sum + (transaction.totalAmountDue || 0), 0);
      return { date, totalAmount: total.toFixed(2) }; // Ensure two decimal points
    });

    doc.autoTable({
      head: [['Date', 'Billing Cycle', 'Vendor ID', 'Name', 'Location', 'Amount', 'Reference Number', 'Mode of Payment']],
      body: groupedTotalAmounts.flatMap(({ date, totalAmount }, index) => [
        [{ content: `Date: ${date} - Total: ₱${totalAmount}`, colSpan: 8, styles: { halign: 'left', fontStyle: 'bold' } }],
    ...
    doc.text(`Total Amount: ₱${totalAmount.toFixed(2)}`, 105, doc.autoTable.previous.finalY + 10, { align: "center" });
        ...
        {!loading && !error && filteredTransactions.length > 0 && (
          <>
            {Object.entries(groupedTransactions).map(([date, { transactions, total }]) => (
              <div key={date}>
                <DateTotalHeader>
                  {date} <span>Total: ₱{total.toFixed(2)}</span> {/* Ensure two decimal points */}
                </DateTotalHeader>
                <Table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {transactions.map((transaction, index) => (
                      <tr key={index}>
                        ...
                        <td onClick={() => handleNameClick(transaction.vendorID)}>
                          {transaction.name}
                        </td>
                        <td>{transaction.billingCycle}</td>
                        <td>₱{(transaction.totalAmountDue || 0).toFixed(2)}</td> {/* Ensure two decimal points */}
                        <td>{transaction.referenceNumber}</td>
                        <td>{transaction.paidBy || 'gcash'}</td> {/* Add the new table data with default 'gcash' */}
                      </tr>
                    ))}
                  </tbody>
                </Table>
