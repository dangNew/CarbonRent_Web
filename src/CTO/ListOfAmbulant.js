...
const ListsOfAmbulant = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const ambulantQuery = query(collection(rentmobileDb, 'payment_ambulant'));
        const ambulantSnapshot = await getDocs(ambulantQuery);

        const collectorQuery = query(collection(rentmobileDb, 'ambulant_collector'));
        ...
        const ambulantData = ambulantSnapshot.docs.map(doc => {
          ...
          return transaction.zone !== 'N/A' && transaction.total_amount !== 'N/A';
        });
  ...
  const filterTransactions = (dateValue, filterType) => {
    ...
    if (dateValue) {
      filtered = filtered.filter(transaction =>
        formatDate(transaction.date) === dateValue
      );
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter(transaction =>
        collectors[transaction.collector]?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    if (adminLocation) {
      filtered = filtered.filter(transaction => {
        const collectorLocation = collectorLocations[transaction.collector + '_location'];
        return collectorLocation && collectorLocation !== 'N/A' && collectorLocation === adminLocation;
      });
    }

    if (collector) {
      filtered = filtered.filter(transaction => collectors[transaction.collector] === collector);
    }
  ...
  const generatePDF = async () => {
    ...
    if (cebuMarketImageBase64) doc.addImage(cebuMarketImageBase64, "PNG", 160, 10, 30, 30);
    ...
    doc.autoTable({
      head: [['Zone', 'Date', 'Space Rate', 'No. of Ticket', 'Amount', 'Reference Number', 'Collector', 'Location']],
  ...
  return (
    <DashboardContainer>
      ...
      <FiltersContainer>
        ...
        <FilterButton active={filter === 'Recent'} onClick={() => handleFilterChange('Recent')}>Recent</FilterButton>
        <FilterButton active={filter === 'This Week'} onClick={() => handleFilterChange('This Week')}>This Week</FilterButton>
        <DateInput type="date" value={displayDate} onChange={handleDateChange} />
      </FiltersContainer>
      <TableContainer>
        {loading ? (
          <p>Loading...</p>
            ...
            <tbody>
              {filteredTransactions.map((item, index) => (
                <tr key={index} onClick={() => handleCellClick(item)}>
                  ...
                  <td>{collectorLocations[item.collector + '_location'] || 'N/A'}</td>
                </tr>
              ))}
            </tbody>
          </Table>
