...
const AmbulantHistory = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const ambulantQuery = query(collection(rentmobileDb, 'payment_ambulant'), where('location', '==', adminLocation));
  ...
  const filterTransactions = (dateValue, filterType) => {
    ...
    if (adminLocation) {
      filtered = filtered.filter(transaction => {
        ...
        console.log('Filtering transaction:', transaction, 'Collector location:', collectorLocation, 'Admin location:', adminLocation); // Debugging
  ...
  const groupedTotalAmounts = Object.entries(groupedItems).map(([date, dateItems]) => {
    const total = dateItems.reduce((sum, item) => sum + item.total_amount, 0).toFixed(2);
    ...
    <DashboardContainer>
      ...
      <FiltersContainer>
        <FiltersRightSide>
          ...
          <FilterOption active={filter === 'Recent'} onClick={() => handleFilterChange('Recent')}>Recent</FilterOption>
          <FilterOption active={filter === 'This Week'} onClick={() => handleFilterChange('This Week')}>This Week</FilterOption>
        </FiltersRightSide>
      ...
      <TableContainer>
        {loading ? (
          ...
          Object.entries(groupedItems).map(([date, dateItems]) => {
            const totalAmount = groupedTotalAmounts.find(item => item.date === date)?.totalAmount;
            return (
              <div key={date}>
                <DateTotalHeader>
                ...
                <Table>
                  ...
                  <tbody>
                    {dateItems.map((item, index) => (
                      <TableRow key={index} onClick={() => handleCellClick(item)}>
                        ...
                        <TableData>{collectors[item.collector] || 'N/A'}</TableData>
                        <TableData>{collectorLocations[item.collector + '_location'] || 'N/A'}</TableData>
                      </TableRow>
                    ))}
                  </tbody>
                </Table>
