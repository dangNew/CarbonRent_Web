...
const CollectorList = () => {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      const ambulantCollectorsSnapshot = await getDocs(collection(rentmobileDb, 'ambulant_collector'));
      const appraisalUsersSnapshot = await getDocs(collection(rentmobileDb, 'appraisal_user'));
      const appraisalsSnapshot = await getDocs(collection(rentmobileDb, 'appraisals'));
      const paymentAmbulantSnapshot = await getDocs(collection(rentmobileDb, 'payment_ambulant'));
      ...
      const filteredAmbulantCollectorsData = ambulantCollectorsData.filter((collector) =>
      ...
      appraisalUsersData.forEach((user) => {
        user.totalAmount = appraisalsData
          .filter((appraisal) => appraisal.appraisal === user.appraisal && appraisal.appraisal_assign === user.appraisal_assign)
          .reduce((acc, appraisal) => acc + appraisal.total_amount, 0);
      });

      // Filter ambulant collectors based on admin location
      const filteredAmbulantCollectorsByLocation = filteredAmbulantCollectorsData.filter(
      ...
      setItems([...filteredAmbulantCollectorsByLocation, ...filteredAppraisalUsersData]);
      setLoading(false);
    };
  ...
  return (
    <DashboardContainer>
      ...
      <FilterContainer>
        ...
        />
        <FilterDropdown value={filter} onChange={(e) => setFilter(e.target.value)}>
      ...
      {loading ? (
        ...
        Object.keys(groupedItems).length > 0 ? (
          Object.keys(groupedItems).map((collector) => {
            const totalAmount = groupedItems[collector].reduce((acc, item) => acc + (item.totalAmount || 0), 0);
            ...
            return (
              <CollectorContainer key={collector} onClick={() => handleViewCollector(collector, collectorType, location, zone)} isClickable={true}>
                <CollectorHeader>
                  <CollectorName>{collector}</CollectorName>
                  <CollectorTotal>Total: ₱{totalAmount.toFixed(2)}</CollectorTotal>
                </CollectorHeader>
                <CollectorLabel>
                  {collectorType === 'ambulant' ? 'Collector for Ambulant' : 'Collector for Appraisal'}
                </CollectorLabel>
                <LocationLabel>
                  Location: {location} {collectorType === 'ambulant' ? `- Zone ${zone}` : ''}
                </LocationLabel>
              </CollectorContainer>
            );
          })
