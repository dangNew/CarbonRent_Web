// src/MarketViolation.js
import React, { useEffect, useState } from 'react';
import styled from 'styled-components';
import { rentmobileDb } from '../components/firebase.config'; // Adjust the path if necessary
...
const MarketViolation = () => {
  ...
  const filterViolations = (searchValue, dateValue) => {
    const filtered = violations.filter(violation => {
      ...
      return (
        (violation.vendorName && violation.vendorName.toLowerCase().includes(searchValue.toLowerCase())) &&
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [['Date', 'Stall Location', 'Stall No', 'Vendor ID', 'Vendor Name', 'Violation Type', 'Warning', 'Total Amount']],
