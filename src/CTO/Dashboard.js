...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const ambulantSnapshot = await getDocs(collection(rentmobileDb, 'payment_ambulant'));
        ...
        // Fetch appraisals
        const appraisalsSnapshot = await getDocs(collection(rentmobileDb, 'appraisals'));
        ...
        const combinedData = [...paymentsData, ...ambulantData, ...appraisalsData].map(transaction => ({
          ...transaction,
          location: transaction.type === 'Stall Holder' ? approvedVendorsData[transaction.vendorId]?.location :
                    transaction.type === 'Appraisal' ? transaction.appraisal_assign :
        ...
        const appraisalsTotals = calculateTotals(filteredData.filter(t => t.type === 'Appraisal'), 'Appraisal');
        const ambulantTotals = calculateTotals(filteredData.filter(t => t.type === 'Ambulant Vendor'), 'Ambulant Vendor');
        const stallHoldersTotals = calculateTotals(filteredData.filter(t => t.type === 'Stall Holder'), 'Stall Holder');
    ...
    const fetchViolations = async () => {
      try {
        ...
        setStats(prevStats => ({ ...prevStats, recentViolations: filteredData.length }));
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const filtered = recentTransactions.filter(t => {
      ...
      return transactionDate.getTime() === today.getTime() && matchesSearch && matchesType;
    });
    ...
    <DashboardContainer>
      ...
      {loading ? (
        ...
        <>
          <StatsContainer>
            {[{
              title: "Appraisal", value: computeTotals('Appraisal'), icon: faUsers, redirectPath: "/historyAppraisal"
            }, {
              title: "Ambulant", value: computeTotals('Ambulant Vendor'), icon: faFileInvoice, redirectPath: "/ambulantHistory"
            }, {
              title: "Stall Holder", value: computeTotals('Stall Holder'), icon: faFileInvoice, redirectPath: "/stallHistory"
            }, {
              title: "Violations", value: stats.recentViolations, icon: faExclamationTriangle, redirectPath: "/listViolators"
              ...
              >
                ...
                <StatValue>{title === "Violations" ? value : `₱${Number(value).toFixed(2)}`}</StatValue>
              </StatCard>
            ))}
          </StatsContainer>
          ...
          <FilterContainer>
            ...
            <FilterButton
              className={transactionFilter === 'Ambulant Vendor' ? 'active' : ''}
