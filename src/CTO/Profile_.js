...
import { rentmobileDb, rentmobileStorage, rentmobileAuth } from '../components/firebase.config'; // Import the Firebase configuration
...
const Profile = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (storedUserData && storedUserData.email) {
          const usersRef = collection(rentmobileDb, 'admin_users');
  ...
  const handleSave = async () => {
    try {
      setMessage('Saving...');
      const userRef = doc(rentmobileDb, 'admin_users', userData.id);
      ...
      if (imageFile) {
        ...
        const storageRef = ref(rentmobileStorage, `images/${imageFile.name}`);
      ...
      if (signatureFile) {
        ...
        const storageRef = ref(rentmobileStorage, `signatures/${signatureFile.name}`);
      ...
      if (editedData.email !== userData.email) {
        await updateEmail(rentmobileAuth.currentUser, editedData.email);
      ...
      if (editedData.password && editedData.password !== userData.password) {
        await updatePassword(rentmobileAuth.currentUser, editedData.password);
      }
  ...
  return (
    <ProfileContainer>
      <ProfileHeader>
        {userData.Image ? (
          <ProfileImage src={userData.Image} alt={`${userData.firstName} ${userData.lastName}`} />
        ) : (
          <div style={{ width: '120px', height: '120px', borderRadius: '50%', backgroundColor: '#ddd' }} />
        )}
        <ProfileInfo>
        ...
        {editing ? (
          <>
            ...
            <ProfileDetailItem>
              ...
              <InputField type="email" name="email" value={editedData.email} onChange={handleChange} />
            </ProfileDetailItem>
            ...
            <ProfileDetailItem>
              ...
              <InputField type="text" name="address" value={editedData.address} onChange={handleChange} />
            </ProfileDetailItem>
            <ProfileDetailItem>
              <ProfileLabel>Contact Number:</ProfileLabel>
              <InputField type="text" name="contactNum" value={editedData.contactNum} onChange={handleChange} />
            </ProfileDetailItem>
            <ProfileDetailItem>
