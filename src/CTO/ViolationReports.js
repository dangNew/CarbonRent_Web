...
import { rentmobileDb, rentmobileStorage } from '../components/firebase.config'; // Adjust the path to your Firebase configuration
...
const ViolationReport = ({ show, onHide, violator }) => {
  ...
  useEffect(() => {
    const fetchVendorLocation = async () => {
      try {
        const vendorDocRef = doc(rentmobileDb, 'approvedVendors', violator.vendorID);
    ...
    const fetchViolationsCount = async () => {
      try {
        const violationsQuery = query(
          collection(rentmobileDb, 'Notice_Report'),
    ...
    const fetchAdminData = async () => {
      try {
        ...
        if (storedUserData && storedUserData.email) {
          const adminQuery = query(collection(rentmobileDb, 'admin_users'), where('email', '==', storedUserData.email));
  ...
  const sendToFirestore = async () => {
    try {
      ...
      const storageRef = ref(rentmobileStorage, `violation_reports/${formData.vendorId}_${new Date().getTime()}.png`);
      ...
      await addDoc(collection(rentmobileDb, 'Notice_Report'), reportData);

      // Update the violator data in Firestore to mark the report as sent
      const violatorDocRef = doc(rentmobileDb, 'stall_payment', violator.id);
